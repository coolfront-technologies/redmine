...
# redmine_s3 is gitignored; only patch/install when the plugin is present (e.g. volume mount)
RUN if [ -f plugins/redmine_s3/Gemfile ]; then \
      sed -i "s/gem 'aws-sdk', '~> 2.11'/gem 'aws-sdk-v1'/g" plugins/redmine_s3/Gemfile; \
    fi
...
RUN if [ -f plugins/redmine_s3/Gemfile ]; then \
      cd plugins/redmine_s3 && bundle install --without development test minimagick; \
    fi
