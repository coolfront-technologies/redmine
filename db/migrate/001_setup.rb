...
EXPOSE 3010
CMD ["sh", "-c", "export SECRET_TOKEN=${SECRET_TOKEN:-${SECRET_KEY_BASE}} && echo 'Creating database.yml from DATABASE_URL...' && echo 'production:\n  adapter: postgresql\n  encoding: unicode\n  url: <%= ENV[\"DATABASE_URL\"] %>' > config/database.yml && echo 'Running database migrations...' && bundle exec rake db:migrate RAILS_ENV=production && echo 'Starting Rails server...' && bundle exec rails server -b 0.0.0.0 -p ${PORT:-3010}"]
